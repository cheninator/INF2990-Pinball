...
void main()
{
	...
	vec3 N = normal ;
	if(gl_FrontFacing) N = -N;	// vecteur normal
	...
	vec4 lumiereReflechie[4] = {vec4(0),vec4(0),vec4(0),vec4(0)};
	...
	lumiereReflechie[DIRECTIONNELLE] = composanteAmbiante + composanteDiffuse + composanteSpeculaire;
	...
	float cosDelta = .5;					// cos de l'angle d'ouverture du cone
	...
	composanteDiffuse = NdotL[SPOT] * gl_LightSource[SPOT].diffuse*textureColor; 
	composanteSpeculaire = pow(max(NdotHV[SPOT], 0.0),6.0) * gl_LightSource[SPOT].specular;

	lumiereReflechie[SPOT] += clamp(composanteAmbiante, 0.0, 1.0) ;
	lumiereReflechie[SPOT] += clamp(composanteDiffuse, 0.0, 1.0);
	lumiereReflechie[SPOT] += clamp(composanteSpeculaire, 0.0, 1.0);
